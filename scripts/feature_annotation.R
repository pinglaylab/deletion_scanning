...
haploid_long <- deletions_annot %>% filter(ploidy == "haploid", `SV length (log10)` > 3) %>%
  # 1) make a binary outcome: 1 = post selection, 0 = pre selection
  mutate(is_post = selection == "post selection") %>%
  # 2) pivot chromatin features to long format
  pivot_longer(
    cols = c(`SV length (log10)`,`Accessible heterochromatin`:`lncRNA dispensability (CRISPR)`),
    names_to = "feature",
    values_to = "raw"
  ) %>%
  group_by(ploidy, feature) %>%
  filter(sd(raw, na.rm = TRUE) > 0) %>% # variance filter
  mutate(scaled = as.numeric(scale(raw))) %>%
  ungroup()

diploid_long <- deletions_annot %>% filter(ploidy == "diploid", `SV length (log10)` > 3) %>%
  # 1) make a binary outcome: 1 = post selection, 0 = pre selection
  mutate(is_post = selection == "post selection") %>%
  # 2) pivot chromatin features to long format
  pivot_longer(
...
# For each state, fit: is_post ~ frac + cell_line
log_odds_haploid <- haploid_long %>%
  group_by(feature) %>%
  group_modify(~ {
    m <- glm(is_post ~ scaled + cell_line, data = .x, family = binomial)
    tidy(m)
  }) %>%
  ungroup() %>%
  filter(term == "scaled") %>%
  transmute(
    feature,
    log_odds = estimate,      # common log-odds across all cell lines
    se       = std.error,
    z        = statistic,
    p_value  = p.value) %>%
  mutate(FDR = p.adjust(p_value, method = "BH"),
         sig = ifelse(FDR < 0.05, "FDR < 0.05", "non significant"),
         ci95_lo = log_odds - 1.96 * se,
         ci95_hi = log_odds + 1.96 * se,
         ploidy = "haploid",
         OR = exp(log_odds),
         OR_lo = exp(ci95_lo),
         OR_hi = exp(ci95_hi))

log_odds_diploid <- diploid_long %>%
  group_by(feature) %>%
  group_modify(~ {
    m <- glm(is_post ~ scaled + cell_line, data = .x, family = binomial)
    tidy(m)
...
# with CRISPR as co-variate
haploid_long_CRISPR <- deletions_annot %>% filter(ploidy == "haploid", `SV length (log10)` > 3) %>%
  # 1) make a binary outcome: 1 = post selection, 0 = pre selection
  mutate(is_post = selection == "post selection") %>%
  # 2) pivot chromatin features to long format
  pivot_longer(
    cols = c(`SV length (log10)`,`Accessible heterochromatin`:`lncRNA dispensability (CRISPR)`, -`Coding gene dispensability (CRISPR)`),
    names_to = "feature",
    values_to = "raw"
  ) %>%
  group_by(ploidy, feature) %>%
  filter(sd(raw, na.rm = TRUE) > 0, feature != "Coding gene dispensability (Depmap)") %>% # variance filter
  mutate(scaled = as.numeric(scale(raw))) %>%
  ungroup()

diploid_long_CRISPR <- deletions_annot %>% filter(ploidy == "diploid", `SV length (log10)` > 3) %>%
  # 1) make a binary outcome: 1 = post selection, 0 = pre selection
  mutate(is_post = selection == "post selection") %>%
  # 2) pivot chromatin features to long format
  pivot_longer(
...
# For each state, fit: is_post ~ frac + cell_line + CRISPR
log_odds_haploid_CRISPR <- haploid_long_CRISPR %>%
  group_by(feature) %>%
  group_modify(~ {
    m <- glm(is_post ~ scaled + cell_line + `Coding gene dispensability (CRISPR)`, data = .x, family = binomial)
    tidy(m)
  }) %>%
  ungroup() %>%
  filter(term == "scaled") %>%
  transmute(
    ...
    z        = statistic,
    p_value  = p.value) %>%
  mutate(sig = ifelse(p_value < 0.05, "significant", "non significant"),
         ci95_lo = log_odds - 1.96 * se,
         ci95_hi = log_odds + 1.96 * se,
         OR = exp(log_odds),
         OR_lo = exp(ci95_lo),
...
log_odds_diploid_CRISPR <- diploid_long_CRISPR %>%
  group_by(feature) %>%
  group_modify(~ {
    m <- glm(is_post ~ scaled + cell_line + `Coding gene dispensability (CRISPR)`, data = .x, family = binomial)
    tidy(m)
...
# ==== Non-coding deletions ====
# 1 feature
haploid_long_noncoding <- deletions_annot %>% filter(ploidy == "haploid", `SV length (log10)` > 3, `Protein coding` == 0) %>%
  # 1) make a binary outcome: 1 = post selection, 0 = pre selection
  mutate(is_post = selection == "post selection") %>%
  # 2) pivot chromatin features to long format
  pivot_longer(
...
diploid_long_noncoding <- deletions_annot %>% filter(ploidy == "diploid", `SV length (log10)` > 3, `Protein coding` == 0) %>%
  mutate(is_post = selection == "post selection") %>%
  pivot_longer(
    cols = c(`SV length (log10)`,`Accessible heterochromatin`:`lncRNA dispensability (CRISPR)`),
    names_to = "feature",
...
# For each state, fit: is_post ~ frac + cell_line
log_odds_haploid_noncoding <- haploid_long_noncoding %>%
  group_by(feature) %>%
  group_modify(~ {
    m <- glm(is_post ~ scaled + cell_line, data = .x, family = binomial)
    tidy(m)
...
log_odds_diploid_noncoding <- diploid_long_noncoding %>%
  filter(!is.nan(scaled)) %>%
  group_by(feature) %>%
  group_modify(~ {
    m <- glm(is_post ~ scaled + cell_line, data = .x, family = binomial)
    tidy(m)
